...
import { Plus, BarChart3, Calendar, FileText, Building2, TrendingUp, LogOut } from "lucide-react";
    ...
    <div className="flex min-h-screen bg-background text-foreground">
      ...
      <div className="w-64 bg-sidebar border-r border-sidebar-border fixed h-full overflow-y-auto">
        <div className="p-4 border-b border-sidebar-border">
          <h2 className="text-lg font-bold text-sidebar-foreground">Netpoleon Admin</h2>
              ...
              <li key={item.href}>
                {item.disabled ? (
                  <div className="flex items-center px-3 py-2 text-sidebar-foreground/50 rounded-md border-l-2 border-transparent cursor-not-allowed text-sm">
                    <span className="mr-2 text-sidebar-foreground/50">{item.icon}</span>
                    {item.label}
                  </div>
                ) : (
                  <Link 
