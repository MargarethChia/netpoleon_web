...
import { MoreHorizontal, Eye, Edit, Trash2, Search, FileText, Calendar, Star, Image } from "lucide-react";
    ...
    try {
      ...
      if (resource) {
        await resourcesApi.update(resourceId, {
          ...
          published_at: !resource.is_published ? new Date().toISOString().split('T')[0] : null
        })
      ...
      <div className="space-y-6 animate-in fade-in-0 slide-in-from-bottom-4 duration-700">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Find specific resources by title or content</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {filteredResources.length} resources total • {filteredResources.filter(r => r.is_published).length} published • {featuredResources.length} featured
            </CardDescription>
            <div className="text-sm text-muted-foreground mt-2">
              💡 Only one resource can be featured at a time. Selecting a new featured resource will automatically remove the previous one.
            </div>
          </CardHeader>
          <CardContent>
            {filteredResources.length === 0 ? (
              <div className="text-center py-8 text-muted-foreground">
                {searchTerm ? "No resources found matching your search." : "No resources found. Create your first resource!"}
              </div>
            ) : (
              <Table>
                <TableHeader>
                    ...
                    <TableRow key={resource.id}>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          ...
                          {resource.published_at ? new Date(resource.published_at).toLocaleDateString() : 'Not published'}
                        </div>
                      </TableCell>
                      <TableCell className="text-center">
                        <Button
                          variant={isFeatured(resource.id) ? 'default' : 'outline'}
                          size="sm"
                          onClick={() => handleToggleFeatured(resource.id)}
                          className="mx-auto"
                        >
                          <Star className={`h-4 w-4 ${isFeatured(resource.id) ? 'text-yellow-500' : ''}`} />
                        </Button>
                      </TableCell>
                      <TableCell>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem onClick={() => handleTogglePublish(resource.id)}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-card-foreground">{resources.length}</div>
