...
vi.mock('@/lib/storage', () => ({
  uploadImage: vi.fn().mockResolvedValue({ success: true, url: 'https://cdn.test/img.jpg' })
}))
...
describe('Admin Resources Page', () => {
  ...
  it('delete handles API error without crashing', async () => {
    seed(); del.mockRejectedValue(new Error('Delete failed'))
    render(<ResourcesPage />)
    await screen.findByText(/all resources/i)
    const row = screen.getByRole('row', { name: /xdr ebook/i })
    const menuBtn = within(row).getByTestId('row-menu')
  ...
  it('create page: cover image preview appears and remove hides it', async () => {
    ...
    await user.type(screen.getByLabelText(/or enter image url manually/i), 'https://cdn.test/c.jpg')
  ...
  it('edit page: existing cover image renders and remove hides it', async () => {
    ...
    EditResourcePage = (await import('../admin/resources/[id]/edit/page')).default
